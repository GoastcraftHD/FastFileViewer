...
PhysicalDevices::PhysicalDevices(const VkInstance& instance, const VkSurfaceKHR& surface)
{
    ...
    for (U32 i = 0; i < numDevices; i++)
    {
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(
            device, &numQueueFamilies, m_PhysicalDevices[i].QueueFamiliyProperties.data());
        ...
        {
            const VkQueueFamilyProperties& queueFamiliyProperties =
                m_PhysicalDevices[i].QueueFamiliyProperties[j];

            queueFamilyDebugSs << "                " << j << ":\n";
            queueFamilyDebugSs << "                  GFX: "
                               << ((queueFamiliyProperties.queueFlags & VK_QUEUE_GRAPHICS_BIT)
                                       ? "True"
                                       : "False")
                               << "\n";
            queueFamilyDebugSs << "                  Compute: "
                               << ((queueFamiliyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT)
                                       ? "True"
                                       : "False")
                               << "\n";
            queueFamilyDebugSs << "                  Transfer: "
                               << ((queueFamiliyProperties.queueFlags & VK_QUEUE_TRANSFER_BIT)
                                       ? "True"
                                       : "False")
                               << "\n";
            queueFamilyDebugSs << "                  Sparse binding: "
                               << ((queueFamiliyProperties.queueFlags & VK_QUEUE_SPARSE_BINDING_BIT)
                                       ? "True"
                                       : "False")
                               << "\n";

            FFV_CHECK_VK_RESULT(vkGetPhysicalDeviceSurfaceSupportKHR(
                device, j, surface, &(m_PhysicalDevices[i].QueueSupportsPresent[j])));
        }

        U32 numFormats = 0;
        FFV_CHECK_VK_RESULT(
            vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &numFormats, VK_NULL_HANDLE));

        FFV_ASSERT(numFormats > 0, "", exit(1));

        m_PhysicalDevices[i].SurfaceFormats.resize(numFormats);

        FFV_CHECK_VK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(
            device, surface, &numFormats, m_PhysicalDevices[i].SurfaceFormats.data()));
        ...
        FFV_CHECK_VK_RESULT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
            device, surface, &(m_PhysicalDevices[i].SurfaceCapabilities)));
        ...
        imageUsageFlagsDebugSs << "                Transfer src: "
                               << ((imageUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT) ? "True"
                                                                                       : "False");
        imageUsageFlagsDebugSs << "\n                Transfer dst: "
                               << ((imageUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT) ? "True"
                                                                                       : "False");
        imageUsageFlagsDebugSs << "\n                Sampled: "
                               << ((imageUsageFlags & VK_IMAGE_USAGE_SAMPLED_BIT) ? "True"
                                                                                  : "False");
        imageUsageFlagsDebugSs << "\n                Color attachment: "
        ...
        FFV_CHECK_VK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(
            device, surface, &numPresentModes, VK_NULL_HANDLE));
        ...
        FFV_CHECK_VK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(
            device, surface, &numPresentModes, m_PhysicalDevices[i].PresentModes.data()));
        ...
        for (U32 j = 0; j < m_PhysicalDevices[i].MemoryProperties.memoryTypeCount; j++)
        {
            ...
            memoryTypeDebugSs << " Heap: "
                              << m_PhysicalDevices[i].MemoryProperties.memoryTypes[j].heapIndex
                              << "\n";
        }
        ...
        FFV_TRACE("Device Index: {0}\n"
                  ...
                  i, m_PhysicalDevices[i].DeviceProperties.deviceName,
                  ...
                  numQueueFamilies, queueFamilyDebugSs.str(), formatsDebugSs.str(),
                  imageUsageFlagsDebugSs.str(), numPresentModes,
                  m_PhysicalDevices[i].MemoryProperties.memoryTypeCount, memoryTypeDebugSs.str(),
